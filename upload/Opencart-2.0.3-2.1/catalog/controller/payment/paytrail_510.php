...
class ControllerPaymentPaytrail510 extends Controller {
  ...
  private $checkout_secret = '';
    ...
    $data['back'] = $this->url->link('checkout/cart');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paytrail_510.tpl')) {
			  return $this->load->view($this->config->get('config_template') . '/template/payment/paytrail_510.tpl', $data);
		} else {
		  	return $this->load->view('default/template/payment/paytrail_510.tpl', $data);
		}
	  }
    ...
    protected function getBody(){
       ...
       $products = round($product_total,2);
        ...
        if($products == $order_info['total'] && $summa > 0){
     ...
     if(!$error){  
       $body =  json_encode(array(
                    ...
                    'callbackUrls' => array(
                       'success' => $this->url->link('paytrail_510/complete'),
                       'cancel' => $this->url->link('payment/paytrail_510/cancel')
                    )
              ),
  ...
  public function complete() {
     ...
     if($order_id && $str_len > 20 && $amount > 0 && $status == 'ok'){
        ...
        if($order_info['order_status_id'] != $this->config->get('paytrail_510_order_status_id')){
            ...
            $this->db->query("UPDATE `" . DB_PREFIX . "order` SET `payment_method` = '" . $this->language->get('text_title') . " (" . $provider . ")', date_modified = NOW() WHERE order_id = '" . (int)$this->session->data['order_id'] . "'");

            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('paytrail_510_order_status_id'));
        ...
        $this->response->redirect($this->url->link('checkout/success', true));   
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('paytrail_510_order_cancel_status_id'));
