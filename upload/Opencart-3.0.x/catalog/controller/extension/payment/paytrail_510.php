<?php
class ControllerExtensionPaymentPaytrail510
 extends Controller {
  ...
  private $paytrail_secret = '';
    ...
    require_once(DIR_SYSTEM . 'library/paytrail_510.php');
        ...
        if($this->config->get('payment_paytrail_510_sandbox')){
           $this->paytrail_merchant = 375917;
           $this->paytrail_secret = 'SAIPPUAKAUPPIAS';
        } else {
            $this->paytrail_merchant = trim($this->config->get('payment_paytrail_510_merchant'));
            $this->paytrail_secret = trim($this->config->get('payment_paytrail_510_security'));
        }
        ...
        if(isset($checked['error'])){
          ...
          $error = true;
               ...
               $logs = new Log('paytrail_error.log');
               $logs->write($string);
        }

        if(!$error){
           $paytrail = new Paytrail_510($this->paytrail_merchant,$this->paytrail_secret, $this->getHeader(), $this->getBody());
		...
		return $this->load->view('extension/payment/paytrail_510', $data);
	}
  ...
  public function getBody(){
        ...
        $this->load->model("extension/payment/paytrail_510");
        $this->load->model('checkout/order');
     ...
     if(!$error){     
        $body =  json_encode(array(
                    ...
                    'redirectUrls' => array(
                       'success' => $this->url->link('extension/payment/paytrail_510/complete'),
                       'cancel' => $this->url->link('extension/payment/paytrail_510/cancel')
                    ),
                    'callbackUrls' => array(
                       'success' => $this->url->link('extension/payment/paytrail_510/complete'),
                       'cancel' => $this->url->link('extension/payment/paytrail_510/cancel')
  ...
  protected function calculateHmac($params, $body){
    ...
    public function complete() {
     ...
     if($order_id && $str_len > 20 && $amount > 0 && $status == 'ok'){
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_paytrail_510_order_status_id'));
        ...
        $this->response->redirect($this->url->link('checkout/success', true));   
            ...
            $this->db->query("UPDATE `" . DB_PREFIX . "order` SET `payment_method` = '" . $this->language->get('text_title') . " (" . $provider . ")', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_paytrail_510_order_cancel_status_id'));
  ...
  public function cancel() {
     ...
     if($order_id && $str_len > 20 && $amount > 0 && $status == 'ok'){
        ...
        $this->response->redirect($this->url->link('checkout/success', true));   
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_paytrail_510_order_cancel_status_id'), "", true);
